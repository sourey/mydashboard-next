...
export default function Home() {
  return (
    <main className="min-h-screen bg-neo-bg relative overflow-hidden" itemScope itemType="https://schema.org/WebPage">
      ...
      <div className="fixed top-20 left-10 w-16 h-16 bg-neo-accent border-4 border-neo-border rotate-12 animate-float opacity-60 hidden lg:block" aria-hidden="true" />
      <div className="fixed top-40 right-20 w-12 h-12 bg-neo-primary border-4 border-neo-border rounded-full animate-float-delayed opacity-60 hidden lg:block" aria-hidden="true" />
      <div className="fixed bottom-40 left-20 w-20 h-20 bg-neo-secondary border-4 border-neo-border animate-spin-slow opacity-40 hidden lg:block" style={{ clipPath: 'polygon(50% 0%, 100% 100%, 0% 100%)' }} aria-hidden="true" />
      <div className="fixed top-1/2 right-10 w-8 h-32 bg-neo-purple border-4 border-neo-border animate-float opacity-50 hidden lg:block" aria-hidden="true" />
